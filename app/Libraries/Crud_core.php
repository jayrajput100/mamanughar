...
class Crud_core {
    ...
    $request, $files = [],   //when current_values() is executed all fields that have type 'file' or 'files' will be stored here
    $multipart       = false, $validator = false;
        ...
        if (isset($params['fields']) && $params['fields']) {
            ...
            foreach ($this->fields as $key => $field) {
                ...
                if (isset($field['relation']) && isset($field['relation']['save_table'])) {
                    $newSchema = ['Field' => $key, 'Type' => 'text', 'Key' => '', 'Default' => '', 'Extra' => 'other_table'];
                ...
                if (isset($field['files_relation']) && isset($field['files_relation']['files_table'])) {
                    $newSchema = ['Field' => $key, 'Type' => 'text', 'Key' => '', 'Default' => '', 'Extra' => 'file_table'];
                    $this->schema[] = (object)$newSchema;
                }
            }
        }
    ...
    function parent_form() {
        ...
        if (isset($post['form'])) {
            ...
            if ($_FILES && isset($_FILES['files'])) unset($_FILES['files']);
            // echo '<pre>';
            // print_r($_FILES);
            // echo '<pre>';
            // exit;
            ...
            foreach ($this->fields as $field => $params) {
                if (($this->action == 'add' && !isset($params['only_edit']) && @$params['only_edit'] !== TRUE) || ($this->action == 'edit' && !isset($params['only_add']) && @$params['only_add'] !== TRUE)) {
                    ...
                    if (isset($params['type']) && ($params['type'] == 'file' || $params['type'] == 'files')) {
                        ...
                        if (isset($params['required']) && $params['required'] === TRUE) {
                            if ($multi) {
                                ...
                                $unsets[] = $field;
                            } else $fileFieldName = $field;
                            $fileRulesArr[] = 'uploaded[' . $fileFieldName . ']';
                        }
                        if (isset($params['max_size'])) $fileRulesArr[] = 'max_size[' . $field . ',' . $params['max_size'] . ']';
                        if (isset($params['is_image']) && $params['is_image'] === TRUE) $fileRulesArr[] = 'is_image[' . $field . ']';
                        if (isset($params['ext_in'])) $fileRulesArr[] = 'ext_in[' . $field . ',' . $params['ext_in'] . ']';
                    ...
                    if ($otherTable) {
                        ...
                        $novalidation = false;
                        $otherTableValues[$otherTable] = ['parent_field' => $relOptions['parent_field'], 'child_field' => $relOptions['child_field'], 'values' => $post[$field]??[],
            ...
            if ($this->validator->withRequest($this->request)->run() || $novalidation) //|| empty($this->fields)
            {
                ...
                if ($file_fields) {
                    foreach ($file_fields as $file_field) {
                        ...
                        if ($fileFieldOptions['type'] == 'file') {
                            ...
                            if ($uploadedFileName) $post[$file_field] = $uploadedFileName;
                        } elseif ($fileFieldOptions['type'] == 'files') $filesData[$file_field] = $fileFieldOptions;
                    }
                }
                if ($this->action == 'add') {
                    ...
                    if (!$otherTables && !$filesData) {
                        if ($affected == 1) $this->flash('success', 'Successfully Updated');
                        else $this->flash('warning', 'The record was not updated or no changes were made');
                    }
                }
                if ($otherTables) {
                    foreach ($otherTables as $otherTable) {
                        ...
                        if ($toInsert) {
                            foreach ($toInsert as $toInsertItem) {
                                $newTempRelationData = [];
                                $newTempRelationData[] = [$otherTableValues[$otherTable]['parent_field'] => $this->id, $otherTableValues[$otherTable]['child_field'] => $toInsertItem];
                                $this->model->batchInsert($otherTable, $newTempRelationData);
                            }
                        }
                        if (!$filesData) {
                            if ($toDelete || $toInsert || $affected) $this->flash('success', 'Successfully Updated');
                            else $this->flash('warning', 'The record was not updated or no changes were made');
                        }
                    }
                    // $otherTableValues = [
                ...
                if ($insertedFilesAffectedRows || $affected || $toDelete || $toInsert) $this->flash('success', 'Successfully Updated');
                else $this->flash('warning', 'The record was not updated or no changes were made');
        ...
        foreach ($this->schema as $field) {
            ...
            $helperText = '';
            if (isset($fields[$f->Field]['helper'])) $helperText = '<small  class="form-text text-muted">' . $fields[$f->Field]['helper'] . '</small>';
            $class = "col-sm-12";
            if (isset($fields[$f->Field]['class'])) $class = $fields[$f->Field]['class'];
            $hidden = false;
            if (isset($fields[$f->Field]['type']) && $fields[$f->Field]['type'] == 'hidden') $hidden = true;
            else {
    ...
    protected function get_label($field, $default_label = '') {
        ...
        if (!isset($field->Type)) return $field['label']??ucfirst(str_replace('_', ' ', $default_label));
    ...
    protected function field_file($field_type, $label, $field_params) {
        ...
        if (isset($field_params['wrapper_start']) && isset($field_params['wrapper_end'])) {
            ...
            $htmlFileName = '';
            $deleteButton = '';
            if ($field_params['show_file_names']??false) $htmlFileName.= '<div class="file-name-wrapper text-center">' . $fileName . '</div>';
                    ...
                    <div class="container">
                        <div class="page-header">
                            <div class="page-title">
                                <h3>' . $this->table_title . '</h3>
                                <div class="crumbs">
                                  <ul id="breadcrumbs" class="breadcrumb">
                                    <li><a href="#"><i class="flaticon-home-fill"></i></a></li>
                                    <li><a href="#">' . $this->table_title . '</a></li>
                                ...
                                <div class="card card-theme">
                                    <div class="card-heading">
                                        <div class="portlet-title portlet-warning d-flex justify-content-between">
                                            <div class="caption  align-self-center">
                                               <h4 class="card-title"> <span class="caption-subject text-uppercase white ml-1"> <i class="fa fa-list"></i> List of ' . $this->table_title . '</span></h4>
                                            </div>
                                            
                                            <div class="actions align-self-center">
                                                <a class="btn btn-fill btn-fill-warning mr-2" href='.base_url($this->base).'/add> <i class="fa fa-plus"></i> Add '.$this->table_title.' </a>
                                            </div>
                                        </div>
                                    ...
                                    <div class="card-body">
                                        <div class="table-responsive">            
                                            <table class="table table-striped table-bordered table-hover" id="html5-extension" style="width:100%"><thead>';
                                            if ($columns) {
                                            ...
                                            if ($columns) {
                                                    ...
                                                    if (is_array($column)) {
                                                        ...
                                                        if (isset($fields[$column]['type']) && strpos($fields[$column]['type'], 'date') !== FALSE) {
                                                            ...
                                                            foreach ($this->schema as $field_types) {
                                                                if ($field_types->Field != $column) continue;
                                                                if (strpos($field_types->Type, 'date') !== FALSE) $field_type = 'date';
                                                ...
                                                foreach ($this->schema as $item) {
                                                    if (strpos($item->Type, 'date') !== FALSE) $field_type = 'date';
                                                    else $field_type = 'text';
                                                    $label = $fields[$item->Field]['label']??ucfirst(str_replace('_', ' ', $item->Field));
                                            ...
                                            $table.= '<td class="text-center"><input class="btn  btn-default btn-sm" type="submit" value="Search"></td></tr></form>';
                                            // Result items
                                                        ...
                                                        if ($columns) {
                                                                ...
                                                                if (is_array($column)) {
                                                                    ...
                                                                    if ($relItems) {
                                                                        ...
                                                                        foreach ($relItems as $relItem) {
                                                                            ...
                                                                            } else $tempRelName[] = $relItem->{$relation['display']};
                                                                        ...
                                                                        } else $display_val = $item->{$relation['display']};
                                                                    }
                                                                } else $display_val = $item->{$column};
                                                            ...
                                                            foreach ($this->schema as $column) {
                                                                ...
                                                                if ($relation) {
                                                                    ...
                                                                    } else $display_val = $item->{$fields[$col_name]['relation']['display']};
                                                                } else $display_val = $item->{$col_name};
                                                                $table.= '<td>' . $display_val . '</td>';
                                                            }
                                                        }
                                                        $table.= '<td class="text-center">
                                                            <div class="btn-group">
                                                                ...
                                                                <div class="dropdown-menu" aria-labelledby="dropdownMenuReference1">
                                                                    <form method="post" action="' . $this->base . '/view" target="_self">
                                                                    <input type="hidden" name="id" id="id" value="' . $item->{$primary_key} . '"><button class="dropdown-item" type="submit"><span class="far fa-eye"></span> View</button>
                                                                    </form>';   
                                                                if($this->table!='pack_gallery' && $this->table!='pack_catalog')
                                                                { 
                                                                 $table.='<form method="post" action="' . $this->base . '/edit" target="_self">
                                                                    <input type="hidden" name="id" id="id" value="' . $item->{$primary_key} . '"><button class="dropdown-item" type="submit"><span class="fas fa-pencil-alt"></span> Edit</button>
                                                                    </form>'; 
                                                                }
                                                                $table.= '<a class="dropdown-item" href="#" onclick="row_delete(' . $item->{$primary_key} . ',&quot;' . $this->table . '&quot,&quot;' . $this->table_title . '&quot,&quot;' . $this->base . '&quot,&quot;' . $primary_key . '&quot)"><span class="far fa-trash-alt"></span>  Delete</a>
                                                                </div>                                    
                                                            </div>
                                                        </td>';
                                                $table.= '</tr>';
                                            ...
                                            $table.= '</tbody></table>';
                                            $table.= '<br/><div class=" clearfix">';
                                            if ($this->request->getPost('table_search')) {
                                                $table.= '<a href="' . $this->base . '/list  " class="btn btn-warning btn-xs"><i class="fa fa-times"></i> Clear filters</a>';
                                            } else {
                                                $table.= $pagination;
                                            }
                                            $table.= '</div>
        ...
        if ($files = $this->request->getFiles()) {
            foreach ($files[$fileFieldName] as $file) {
                if ($file->isValid() && !$file->hasMoved()) {
                    ...
                    $newFilesData[] = [$fileRelationOptions['parent_field'] => $this->id, $fileRelationOptions['file_name_field'] => $file->getName(), $fileRelationOptions['file_type_field'] => $file->getClientMimeType(), ];
                } else return false;
            }
        }
        if ($newFilesData) return $this->model->batchInsert($fileRelationOptions['files_table'], $newFilesData);
        return false;
    }
    ...
    public function getFiles($field = false) {
        if (!$field) return $this->files;
        if (isset($this->files[$field])) return $this->files[$field];
        return false;
    }
    public function deleteItem($table, $where) {
        $item = $this->model->getItem($table, $where);
        if ($item) $this->model->deleteItems($table, $where);
        return $item;
    }
    ...
    public function getFields($field = false) {
        ...
        if ($field = $this->fields[$field]??false) return $field;
        return false;
    }
}