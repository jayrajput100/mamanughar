		...
		<!-- /page_header -->
	     ...
	     if(isset($supplier))
	     {
	     	 ...
	     	 { 
		    	...
		    	{
		    		$supplier1[]=$simple->fetch_supplier_details1($value['supplier_id']);
	                 
	            }
	            foreach ($supplier1 as $key => $value) 
	            {
      	 	...
      	 	?>
    <input type="hidden" name="third_category_id" id="third_category_id" value="<?php echo $third_category_id ?>">  	 	
           ...
           <aside class="col-lg-3" id="sidebar_fixed" style="position: relative; overflow: visible; box-sizing: border-box; min-height: 1px;">
				...
				<div class="theiaStickySidebar" style="padding-top: 0px; padding-bottom: 1px; position: static; transform: none;"><div class="clearfix">
					<div class="sort_select">
							<select name="sort" id="sort" onchange="filter_third_sub_cat()">
					...
					<div class="filter_col">
                                      ...
                                      $checked="checked"; 
										...
										<li>
									        <label class="container_check"><?php echo $value ?><small></small>
									            <input type="checkbox" name="business_category[]" value="<?php echo $key ?>" onclick="filter_third_sub_cat()" <?php echo $checked ?>>
						...
						<div class="filter_type">
							<h4><a href="#filter_1" data-toggle="collapse" class="opened">Products</a></h4>
							<div class="collapse show" id="filter_1">
								...
								<ul>
								 <?php 
								   ...
								   {
								   	if(is_array($third_sub_cat) && count($third_sub_cat)>=1)
								   	{
								   		foreach ($third_sub_cat as $key => $value) 
								   		{ ?>
								   		<li>
									        <label class="container_check"> <?php echo $value['third_subcategory_name'] ?><small></small>
									            <input type="checkbox" name="third_subcategory_id[]" value="<?php echo $value['third_subcategory_id'] ?>" <?php echo ($value['third_subcategory_id']==$third_category_id)?"checked":'' ?> onclick="filter_third_sub_cat()">
									            <span class="checkmark"></span>
									        </label>
								        </li>
	            ...
	            <div class="strip">
	                <figure>
	                    <span class="ribbon off"><?php echo $simple->fetch_city_name($value['city_id'])  ?></span>
                         ...
                         <?php } else{ ?>
                             <img src="<?= base_url();?>/public/front-theme/img/mag_icon.png" data-src="<?= base_url();?>/public/front-theme/img/mag_icon.png" class="img-fluid lazy loaded" alt="" style="opacity: 1;height: 195px;width: 195px;object-fit: contain;">
                        <?php } ?> 
